...
export function ServicesClient({ initial }: { initial: Row[] }) {
  ...
  async function toggleActive(row: Row) {
    ...
    if (error) toast.error("Error al actualizar");
      ...
      toast.success(row.is_active ? "Desactivado" : "Activado");
      await refresh();
    }
  }
  ...
  async function handleUpdate(row: Row, patch: Partial<Row & { price: string }>) {
    ...
    if (patch.description !== undefined)
      upd.description = (patch.description as string) || null;
    if (patch.duration_min !== undefined)
      upd.duration_min = Number(patch.duration_min);
        ...
        <CardContent className="grid gap-4 pt-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-1.5">
                ...
                <Input
                  ...
                  max={480}
                  step={5}
                  value={newRow.duration_min}
                  onChange={(e) =>
                    setNewRow({ ...newRow, duration_min: Number(e.target.value) })
                  }
                />
              </div>
              <div className="grid gap-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              {newRow.is_active ? "Activo" : "Oculto en la página"}
            </span>
          </div>
          <Button onClick={handleCreate} disabled={saving} className="w-fit">
                ...
                <div>
                  <div className="font-medium">
                    ...
                    <span className="font-normal text-muted-foreground">
                      {formatARS(r.price)} · {r.duration_min} min
                    </span>
                  </div>
                  {r.description && (
                ...
                <Input
                  ...
                  onBlur={(e) => {
                    if (e.target.value !== r.name)
                      handleUpdate(r, { name: e.target.value });
                  }}
                />
                <Input
                  type="number"
                  ...
                  onBlur={(e) => {
                    ...
                    if (v !== r.duration_min)
                      handleUpdate(r, { duration_min: v });
                  }}
                />
                <Input
                  defaultValue={String(r.price)}
                  onBlur={(e) => {
                    if (e.target.value !== String(r.price))
                      handleUpdate(r, { price: e.target.value });
                  }}
                />
              </div>
            </CardContent>
          ...
          <Card>
            <CardContent>
              <Empty
                ...
                description="Agregá tu primer servicio arriba para poder recibir reservas."
              />
            </CardContent>
          </Card>
        )}
