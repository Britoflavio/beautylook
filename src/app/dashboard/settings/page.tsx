    ...
    <div className="grid gap-6 max-w-2xl">
      ...
      {mp === "connected" && (
        <div className="rounded-md bg-green-50 border border-green-200 px-4 py-3 text-sm text-green-800">
          ¡Mercado Pago conectado! Ya podés recibir señas.
        </div>
      )}
      {mp === "error" && (
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <Badge variant={pro?.mp_status === "connected" ? "default" : "secondary"}>
              {pro?.mp_status === "connected" ? "Conectado" : pro?.mp_status === "error" ? "Error" : "Desconectado"}
            </Badge>
          </CardTitle>
          <CardDescription>
            Conectá tu cuenta para cobrar las señas directo a tu cuenta. Nosotros no tocamos el dinero.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <SettingsClient mpStatus={pro?.mp_status ?? "disconnected"} mpUserId={pro?.mp_user_id ?? null} />
        </CardContent>
      </Card>
